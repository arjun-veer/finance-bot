...
def get_financial_tips_with_gemini(user_data, user_question=None):
    ...
    if user_question:
        prompt_template = """
        Based on the following financial summary and additional data, answer the user's question. If the question is not related to finance, respond with "Ask questions related to finance":
        Financial Summary:
        {summary}
        Additional Data:
        {additional_data}
        Question: {question}
        """
        input_variables = ["summary", "additional_data", "question"]
        inputs = {"summary": summary_text, "additional_data": additional_data, "question": user_question}
    else:
        prompt_template = """
        Based on the following financial summary and additional data, provide financial tips and advice. Ensure the response is strictly related to financial topics:
        Financial Summary:
        {summary}
        Additional Data:
        {additional_data}
        """
        input_variables = ["summary", "additional_data"]
        inputs = {"summary": summary_text, "additional_data": additional_data}
...
def main():
    ...
    if option == "Track Expenses":
        ...
        if submit_button and user_question.strip():
            ...
            # Add the user's question to the chat history with a placeholder response
            st.session_state.chat_history.append({"question": user_question, "response": ""})
            save_chat_history(st.session_state.chat_history)

            # Immediately display the new input and placeholder response
            with latest_response_container:
                st.markdown(
                    f"""
                    <div style='background-color:#f8f9fa; padding:10px; margin-bottom:10px; border-radius:10px;'>
            ...
            response = get_financial_tips_with_gemini(load_user_data(), user_question)
